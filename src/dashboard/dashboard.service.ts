...
export class DashboardService {
  ...
  async getFleetPositions(
    ...
    limit = 50,
    fleetId?: string,
  ): Promise<PaginatedResponse<FleetPosition>> {
    const safeLimit = Math.min(limit, MAX_PAGE_SIZE);
    const where = fleetId
      ? { id: fleetId, deletedAt: IsNull() }
      : { deletedAt: IsNull() };
  ...
  private countByStatus(
    ...
    for (const f of fleets)
      counts[f.operationalStatus] = (counts[f.operationalStatus] ?? 0) + 1;
    return counts;
  }
  ...
  private async countRecoveryToday(): Promise<number> {
    ...
    return this.recoveryRepo.count({
      where: { detectedAt: MoreThanOrEqual(start) },
    });
  }
  ...
  private async calcAvgDistanceKm(): Promise<number> {
    ...
    const plans = await this.planRepo.find({
      where: { planDate: today, status: 'active' as never },
    });
